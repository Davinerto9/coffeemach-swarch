...
public class OrdenMantenimientoUI extends JPanel {
    ...
    public OrdenMantenimientoUI(ControladorAlarmas controlador) {
    ...
    private void initUI() {
        ...
        tabla.getColumnModel().getColumn(5).setCellRenderer(new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable t, Object val, boolean sel,
                ...
                switch (estado) {
                    case "RESUELTA":
                        setForeground(new Color(0x3B6D11));
                        break;
                    case "EN_PROCESO":
                        setForeground(new Color(0x185FA5));
                        break;
                    case "CANCELADA":
                        setForeground(new Color(0xA32D2D));
                        break;
                    default:
                        setForeground(new Color(0x888880));
                        break;
                }
                if (sel)
                    setForeground(new Color(0x185FA5));
        ...
        footer.add(statLabel("Pendientes", OrdenMantenimiento.PENDIENTE));
        footer.add(statLabel("En proceso", OrdenMantenimiento.EN_PROCESO));
        footer.add(statLabel("Resueltas", OrdenMantenimiento.RESUELTA));
        footer.add(statLabel("Canceladas", OrdenMantenimiento.CANCELADA));
    ...
    private JLabel statLabel(String nombre, OrdenMantenimiento estado) {
        long count = ordenes.stream().filter(o -> o.getEstado() == estado).count();
        JLabel l = new JLabel(nombre + ": " + count);
        l.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        switch (estado) {
            case RESUELTA:
                l.setForeground(new Color(0x3B6D11));
                break;
            case EN_PROCESO:
                l.setForeground(new Color(0x185FA5));
                break;
            case CANCELADA:
                l.setForeground(new Color(0xA32D2D));
                break;
            default:
